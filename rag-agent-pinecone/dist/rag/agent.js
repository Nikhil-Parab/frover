...
export class RAGAgent {
    ...
    async write(data) {
        try {
            ...
            const vectors = await Promise.all(chunks.map(async (chunk, i) => {
                ...
                console.log(`🧠 Generating embedding for chunk ${i}:`, chunk.substring(0, 100));
                const embedding = await this.embedding.generateEmbedding(chunk);
                console.log(`📊 Embedding dimensions for chunk ${i}:`, embedding.length);
                return {
                    id: chunkId,
                    values: embedding,
                    metadata: {
    ...
    async query(query, options = {}) {
        try {
            ...
            console.log("🎯 Pinecone raw response:", JSON.stringify(searchResult, null, 2));
            ...
            const sources = (searchResult.matches || [])
                ...
                if (!passesThreshold) {
                    console.log(`❌ Match ${m.id} filtered out: score ${m.score} < ${threshold}`);
                }
    ...
    async generateAnswer(query, context) {
        ...
        return `Q: ${query}\n\nBased on retrieved info: ${context.substring(0, 300)}...`;
    }
    /** Custom query for meeting and related meetings */
    async queryMeetingAndRelated(query, meetingId) {
        try {
            ...
            const mainMeeting = matches.find((m) => m.metadata?.originalId === meetingId);
            if (!mainMeeting) {
            ...
            const relatedMeetings = matches.filter((m) => m.metadata?.originalId !== meetingId &&
                m.metadata?.topics?.some((topic) => discussedTopics.includes(topic)));
            return {
                main_meeting: {
