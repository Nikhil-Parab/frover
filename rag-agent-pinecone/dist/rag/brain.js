...
export class RAGBrainAgent {
    ...
    async read(query, options = {}) {
        try {
            ...
            if (typeof query === "object" && query.type) {
                const typeResults = await this.storage.listByType(query.type, options.limit || 10);
    ...
    async update(id, updates) {
        try {
            ...
            await this.clearRelatedCaches(updatedData.type, updatedData.metadata?.category);
            return {
                success: true,
    ...
    async delete(id) {
        try {
            ...
            await this.deleteFromIndex(id);
            if (existing) {
                await this.clearRelatedCaches(existing.type, existing.metadata?.category);
            }
            return {
                success: true,
    ...
    async semanticSearch(query, options) {
        ...
        if (this.pinecone) {
            ...
            const answer = await this.generateIntelligentAnswer(query, sources, options);
            const result = {
                answer,
                sources,
        ...
        const scored = enriched
            .map((e) => {
            const docEmb = e.metadata?.embedding || [];
        ...
        const topN = (options.limit && options.limit > 0 ? options.limit : 10);
    ...
    async generateIntelligentAnswer(query, sources, options) {
        if (sources.length === 0) {
            return `I couldn't find relevant information for "${query}" in my knowledge base.`;
        }
        const context = sources.map((s) => s.content).join("\n\n");
        if (options.responseStyle === "summary") {
            return `Based on ${sources.length} relevant sources: ${context.substring(0, 200)}...`;
        }
        if (options.responseStyle === "detailed") {
            return `Query: ${query}\n\nDetailed analysis based on ${sources.length} sources:\n\n${context}`;
        }
        return `Found ${sources.length} relevant items. Top result (${Math.round((sources[0].score || 0) * 100)}% match): ${sources[0].content.substring(0, 300)}...`;
        // (You can plug an LLM here to synthesize answers if desired.)
    }
    // =====================================================
    // INDEXING
    // =====================================================
    ...
    async indexContent(data) {
        try {
            ...
            const docEmb = await this.embedding.generateEmbedding(docChunks[0] ?? data.content);
            let newMeta = {
                ...data.metadata,
                embedding: docEmb,
                indexed: true,
            ...
            if (this.pinecone) {
                ...
                console.log("ℹ️ Pinecone not configured — stored doc-level embedding in KV");
            }
    ...
    async clearRelatedCaches(type, category) {
        if (type)
            await this.cache.delete(`type:${type}`);
        if (category)
            await this.cache.delete(`category:${category}`);
    }
    // =====================================================
    // BATCH OPS
    // =====================================================
    ...
    getAgentTools() {
        return [
            {
                name: "brain_create",
                description: "Store any type of data in the brain with intelligent indexing",
                parameters: {
                    type: "object",
                    properties: {
            ...
            {
                name: "brain_read",
                description: "Intelligent retrieval from the brain using multiple search strategies",
                parameters: {
                    type: "object",
                    properties: {
                        query: {
