export class StorageService {
    env;
    keyPrefix = "brain:";
    conversationPrefix = "conversation:"; // Backward compatibility
    constructor(env) {
        this.env = env;
    }
    // =====================================================
    ...
    async deleteBrainData(id) {
        ...
        if (existing) {
            ...
            if (existing.metadata?.category) {
                await this.updateCategoryIndex(existing.metadata.category, id, "remove");
            }
        }
        return true;
    }
    /** List entities by type with pagination */
    async listByType(type, limit = 100, offset = 0) {
        // Get type index
        const typeIndex = await this.getTypeIndex(type);
        if (!typeIndex || typeIndex.length === 0)
            return [];
        // Apply pagination
    ...
    async listByCategory(category, limit = 100) {
    ...
    async searchEntities(filters) {
        ...
        const results = [];
        let processed = 0;
        const maxToProcess = Math.min(candidates.length, (filters.limit || 100) * 3); // Process 3x limit for filtering
        for (const id of candidates) {
            ...
            if (filters.tags && filters.tags.length > 0) {
                ...
                const hasRequiredTags = filters.tags.every((tag) => entityTags.includes(tag));
                if (!hasRequiredTags)
                    continue;
            }
            ...
            if (filters.dateRange) {
                ...
                if (createdAt < filters.dateRange.start ||
    ...
    // =====================================================
    /** Update type index for fast filtering */
    async updateTypeIndex(type, id, operation) {
    ...
    async updateCategoryIndex(category, id, operation) {
