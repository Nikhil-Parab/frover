            ...
            if (url.pathname === "/query" && request.method === "POST") {
                try {
                    ...
                    if (readData.options && Array.isArray(readData.options.ids)) {
                        const docs = await Promise.all(readData.options.ids.map((id) => brainAgent.getById(id)));
                        const topics = docs
                            .filter((d) => d.success && d.data?.metadata)
                            .map((d) => d.data?.metadata?.topic ?? "Unknown");
                        return new Response(JSON.stringify({
                            success: true,
                            strategy: "id_lookup",
                    ...
                    const result = await brainAgent.read(readData.query, readData.options);
                ...
                switch (path) {
                    ...
                    case "read": {
                        ...
                        if (data.options && Array.isArray(data.options.ids)) {
                            const docs = await Promise.all(data.options.ids.map((docId) => brainAgent.getById(docId)));
                            const topics = docs
                                .filter((d) => d.success && d.data?.metadata)
                                .map((d) => d.data?.metadata?.topic ?? "Unknown");
                            return new Response(JSON.stringify({
                    ...
                    case "update": {
                        if (request.method !== "PUT" || !id) {
                            return new Response(JSON.stringify({ success: false, message: "Missing id or wrong method" }, null, 2), {
                                status: 400,
                                headers: {
                                    ...corsHeaders,
                                    "Content-Type": "application/json",
                    ...
                    case "bulk": {
                        ...
                        if (subPath === "create" && request.method === "POST") {
                            ...
                            if (!data || !Array.isArray(data.items)) {
                                return new Response(JSON.stringify({ success: false, message: "Invalid items array" }, null, 2), {
                                    status: 400,
                                    headers: {
                                        ...corsHeaders,
                                        "Content-Type": "application/json",
                        ...
                        if (subPath === "delete" && request.method === "POST") {
                            ...
                            if (!data || !Array.isArray(data.ids)) {
                                return new Response(JSON.stringify({ success: false, message: "Invalid ids array" }, null, 2), {
                                    status: 400,
                                    headers: {
                                        ...corsHeaders,
                                        "Content-Type": "application/json",
                        ...
                        return new Response(JSON.stringify({ success: false, message: "Unknown bulk route" }, null, 2), {
                            status: 404,
                            headers: { ...corsHeaders, "Content-Type": "application/json" },
                        });
                    }
